...
which produces the model and data plot shown in `r knitr::asis_output(if (knitr::is_html_output()) { "Figure \\@ref(fig:algo-step-html)"} else { "Figure \\@ref(fig:algo-steps)"})`. 
...
Several computationally intensive steps in `quollr` are implemented in C++ using the `Rcpp` and `RcppArmadillo` packages to improve performance. Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:main-tb-html)', '\\@ref(main-tb-pdf)'))` compares the execution times of these C++ implementations with equivalent R code on the `scurve` dataset, based on $50$ benchmark iterations using the `bench` package [@jim2025]. The largest improvement is observed for `gen_hex_coord_cpp()`, which generates the polygon coordinates of the hexagonal bins and is substantially faster than the corresponding R implementation. Smaller but consistent gains are also achieved for the nearest-neighbour search (`compute_highd_dist()`) and error calculation (`compute_errors()`) functions. In contrast, `calc_2d_dist_cpp()` provides only a modest improvement because the equivalent vectorised R code is already efficient. These optimizations help reduce computation time throughout the modelling workflow, making the package more responsive for interactive visualization and more practical for analyzing larger datasets.
...
<!--convert all to ms-->
<!--compute speed up-->
<!-- write the paragraph regarding that-->
...
fun_efficacy <- fun_efficacy |>
  ...
  mutate(
    ...
    speedup = as.numeric(r) / as.numeric(cpp),
    speedup = round(speedup, 1)
  ) |>
  select(func, cpp, r, speedup) |> 
   mutate(func = gsub("_", "\\\\_", func)) 

```

```{r main-tb-html, eval=knitr::is_html_output()}
fun_efficacy |>
  mutate(func = paste0("<code>", func, "</code>")) |> 
  kable(
    caption   = "Benchmark timings for C++ vs pure R implementations of four performance-critical functions in \\texttt{quollr}, evaluated on the \\texttt{scurve} dataset ($n = 1000$, $p = 7$) using 50 iterations.",
    col.names = c("Function", "C++ time", "R time", "Speedup"),
    linesep   = "",
    escape    = FALSE,
    table.pos = "!ht"
  )
...
The package offers several $2\text{-}D$ visualizations (`r knitr::asis_output(if (knitr::is_html_output()) { "Figure \\@ref(fig:geom-outputs)"} else { "Figure \\@ref(fig:geom-outputs-pdf)"})`), including a full hexagonal grid, a hexagonal grid that matches the data, a full grid based on centroid triangulation, a centroid triangulation grid that aligns with the data, and a triangular mesh for any provided set of points.
...
```{r geom-outputs-pdf, fig.cap="The outputs of \\texttt{geom\\_hexgrid} and \\texttt{geom\\_trimesh} include: (a) a complete hexagonal grid, (b) a hexagonal grid that corresponds with the data, (c) a full grid based on centroid triangulation, and (d) a centroid triangulation grid that aligns with the data.", fig.align='center', fig.pos="!ht", fig.height=5, fig.width=20, eval=knitr::is_latex_output(), fig.alt = "The figure consists of four 2-D panels illustrating hexagonal grids and triangulated meshes. Panel (a) shows a complete hexagonal grid covering a rectangular 2-D region, with all hexagons drawn regardless of whether data are present. Panel (b) shows a hexagonal grid restricted to bins that contain data points, with only those hexagons displayed and gaps where bins are empty. Panel (c) shows a full triangulated mesh constructed from centroids across the entire grid region, with centroids connected by straight line segments to form triangles. Panel (d) shows a triangulated mesh constructed only from centroids corresponding to non-empty bins, aligned with the data layout and leaving open areas where no centroids are present. In all panels, the horizontal and vertical axes represent continuous 2-D coordinates with bounded numeric ranges, and the grids and meshes are drawn with uniform line styles and colors."}
...
```{r scurve-highd-model-detourrpdf, out.width=c("50%", "50%"), fig.cap="Screenshots of the lifted high-dimensional wireframe model from the \\texttt{scurve} UMAP layout using \\texttt{detourr}. Regions with sparse or no data in the UMAP layout are also visible in the lifted model.", fig.alt = "The figure shows several screenshots of 2-D projections of the lifted high-dimensional wireframe model derived from the scurve UMAP layout using detourr. In each screenshot, the horizontal and vertical axes represent projected coordinates with continuous numeric ranges. The wireframe model is displayed as a set of connected line segments, and the scurve data points are shown as individual markers overlaid on the wireframe. The orientation and shape of the wireframe and point cloud differ across screenshots as different projections are shown. In some views, parts of the wireframe extend into areas with few or no data points, leaving visible gaps between the model and the data. All screenshots use consistent color and line styles and have similar aspect ratios.", fig.pos="H", fig.show='hold', eval=knitr::is_latex_output()}
...
We illustrate how to use `quollr` to assess whether this is a reasonable layout. `r knitr::asis_output(if (knitr::is_html_output()) { "Figure \\@ref(fig:limb-hbe-html)"} else { "Figure \\@ref(fig:limb-hbe)"})` shows five alternative layouts, and the HBE plot summarizing the resulting model fits. 
