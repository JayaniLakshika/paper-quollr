...
tr_from_to_df_limb <- tr_from_to_df_limb |>
  mutate(ID = row_number()) |>
  filter(!(ID %in% c(108, 99, 11, 231)))
...
tr_from_to_df_limb <- tr_from_to_df_limb |>
  mutate(ID = row_number()) |>
  filter(!(ID %in% c(298, 34, 48, 326)))


distance_df_small_edges_limb <- distance_limb |>
  filter(distance < benchmark_limb) #|>
  #mutate(ID = row_number()) |>
  #filter(!(ID %in% c(298, 34, 48))) |>
  #select(-ID) #benchmark_limb
