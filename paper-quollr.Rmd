...
The algorithm begins by scaling NLDR data to a standard scale using the `gen_scaled_data()` function. This function standardizes the data with first embedding scale to 0 and 1, The second embedding scales to 0 and the maximum of the second embedding. The resulting output provides the scaled NLDR data and the original limits of embeddings.
...
We can compute the `r2` which is the ratio of the ranges of the original embedding components.
