...
The `scurve` is a $7\text{-}D$ simulated dataset. It is constructed by simulating $1000$ observations.

`scurve_umap` is the UMAP \twoD embedding for `scurve` data with `n_neighbors`$=46$ and `min_dist`$=0.9$. Each data set contains a unique ID column that maps the $7\text{-}D$ point in `scurve` to the corresponding \twoD `scurve_umap`.
