...
prism_scaled_obj2 <- gen_scaled_data(
  data = tsne_prism2)
tsne_prism_scaled2 <- prism_scaled_obj2$scaled_nldr
...
prism_scaled_obj3 <- gen_scaled_data(
  data = tsne_prism3)
tsne_prism_scaled3 <- prism_scaled_obj3$scaled_nldr
...
```{r}
umap_pbmc2 <- triangular_3d_data |>
  select(-ID) |>
  umap(n_neighbors = 15, n_components =  2, 
       metric = "euclidean", min_dist = 0.01, init = "spca")

umap_pbmc2 <- umap_pbmc2 |>
  as_tibble()
