...
`scurve` is a $3\text{-}D$ S-curve data set with additional four noise dimensions which is used to build the model. `scurve_umap` is the UMAP $2\text{-}D$ embedding for `scurve` data set with `n_neighbors` is $15$ and `min_dist` is $0.1$. Each data set contains a unique ID column that mapping `scurve` and `scurve_umap`.
...
```{r, echo=TRUE}
scurve_umap_scaled <- scurve_nldr_obj$scaled_nldr

lim1 <- scurve_nldr_obj$lim1
lim2 <- scurve_nldr_obj$lim2
r2 <- diff(lim2)/diff(lim1) 
```

We can calculate the `r2`, which represents the ratio of the ranges of the original embedding components.
...
Constructing the $2\text{-}D$ model primarily involves (i) binning data, (ii) obtaining bin centroids, and (iii) connecting centroids with line segments to indicate neighbors.
