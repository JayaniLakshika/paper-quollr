...
```{r langevitour-limb-tsne-author-proj, eval=knitr::is_html_output()}
...
```{r langevitour-limb-tsne-best-proj, eval=knitr::is_html_output()}
...
This article is created using \CRANpkg{knitr} [@knitr] and \CRANpkg{rmarkdown} [@rmarkdown] in R with the `rjtools::rjournal_article` template. These `R` packages were used for this work: `cli` [@gabor2025], `dplyr`, `ggplot2`, `interp` (>= 1.1-6), `langevitour`, `proxy`, `stats`, `tibble`, `tidyselect`, `crosstalk`, `plotly`, `kableExtra`, `patchwork`, and `readr`.
