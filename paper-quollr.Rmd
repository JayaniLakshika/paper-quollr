...
```{r overview, eval=knitr::is_latex_output(), fig.cap="algorithm", fig.pos='H'}
...
#### $p\text{-}D$ model visualisation

To evaluate how well the $p\text{-}D$ model captures the underlying structure of the high-dimensional data, we provide a visualisation using the `show_langevitour()` function. This function renders a dynamic projection of both the high-dimensional data and the model using the `langevitour` package.
...
The `show_langevitour()` function then renders the visualisation using the `langevitour` interface, displaying both types of points in a dynamic tour. The `edge_data` input defines connections between neighboring bins (i.e., the hexagonal edges) to visualize the model’s structure.
...
To support interactive evaluation of how well the $p\text{-}D$ model captures the structure of the high-dimensional data, we introduce `show_link_plots()`. This visualisation combines two complementary views: the nonlinear dimension reduction (NLDR) representation and a dynamic tour of the model ovelaid the data in the high-dimensional space. Both views are interactively linked, enabling users to explore.

Before visualisation, the input data must be prepared using the `comb_all_data_model()` function. This function combines the high-dimensional data (`highd_data`), the NLDR data (`nldr_data`), and the bin-averaged high-dimensional model representation (`model_highd`) aligned to the $2-\text{D}$ bin layout (`model_2d`):
...
```{r linkerror, echo=TRUE, eval=knitr::is_html_output(), fig.cap="jkjio"}
...
```{r, echo=TRUE, eval=knitr::is_html_output(), fig.cap=""}
