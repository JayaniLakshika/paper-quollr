...
```{r model-limb, fig.cap="Compare the published $2\\text{-}D$ layout (Figure \\ref{fig:limb-hbe}a) and the $2\\text{-}D$ layout selected (Figure \\ref{fig:limb-hbe}f) by HBE plot (Figure \\ref{fig:limb-hbe}) from the tSNE, UMAP, PHATE, TriMAP, and PaCMAP with different hyper-parameters. The Limb muscle data ($n =  1067$) has seven close different shaped clusters in $10\\text{-}D$.", fig.alt = "The figure shows a 2-D scatter plot of a tSNE layout for the Limb muscle dataset (n=1067) with seven clusters in 10 dimensions. Each point represents one observation, with point colour indicating the cluster label reported in the original paper. The horizontal and vertical axes represent the two tSNE coordinates, both on roughly symmetric numeric scales around zero. Points form several visually distinct clusters, with some local variation visible within clusters. A model overlay with a binwidth of 0.06 shows the local density structure, providing a background grid that highlights differences between clusters without fragmenting the plot.", fig.pos="!ht", fig.width=15, fig.height=10, eval=knitr::is_latex_output()}
