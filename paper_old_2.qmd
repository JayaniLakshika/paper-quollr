...
`s_curve_noise_training` is a $3-D$ S-curve dataset with additional four noise dimensions which is used to train the model. `s_curve_noise_test` is used for testing. `s_curve_noise_umap` is the UMAP $2-D$ embedding for `s_curve_noise_training` dataset. Each dataset contains a unique ID column.  
...
The number of bins along the x and y axes, hexagonal grid starting point, buffer size along the x and y axes, hexagon size, and if `is_rm_lwd_hex = TRUE`, benchmark value to remove low density hexagons are parameters that will be determined within `fit_highd_model`, if they are not provided. They are created as they are needed throughout the following example.  
...
Constructing the $2-D$ model mainly contains (1) preprocessing, (2) binning NLD data, (3) obtain bin centroids/ means, and (4) triangulate bin centroids/ means.

### Preprocessing
<!--Discuss scaling the NLDR data-->

The function `gen_scaled_data()` is used to prepare the NLDR data to fit within the bounds required for regular hexagonal binning, ensuring effective visualization. The user can set up the ratio of the width and height of the hexagon using the `hex_ratio` argument. `hex_ratio` will be determined within `gen_scaled_data()`, if it is not provided.  

```{r, echo=TRUE, eval=FALSE}
s_curve_noise_umap_scaled <- gen_scaled_data(data = s_curve_noise_umap, 
                                             x = "UMAP1", y = "UMAP2", 
                                             hex_ratio = NA)
...
In binning data step, the objective is to perform hexagonal binning and assign NLDR data to regular hexagons. 

**Step 1: Compute hexagonal grid configurations**

In the computation of hexagonal bin configurations, there are mainly two configurations to consider: the number of bins along the x and y axes. The `calc_bins()` function is used to compute these configurations. It uses the hexagonal size, and the buffer amount along the x and y axes.

When computing the number of bins along the x and y axes, the process begins by calculating the range of the respective NLDR component. After that, the range is adjusted to accommodate the buffer amount. Then, the spacing between hexagons is determined based on hexagonal size. Finally, the number of bins along the axis is computed by dividing the adjusted range by the corresponding spacing. 

```{r, echo=TRUE, eval=FALSE}
bin_list <- calc_bins(data = s_curve_noise_umap_scaled, x = "UMAP1", 
                      y = "UMAP2", hex_size = 0.1, buffer_x = NA, buffer_y = NA)

num_bins_x <- bin_list$num_x
num_bins_y <- bin_list$num_y
```

The mains steps for the hexagonal binning algorithm can be executed by the main function `hex_binning()`, or can be run separately for more flexibility. The number of bins along the x and y axes, hexagonal grid starting point, buffer size along the x and y axes, and hexagon size are parameters that will be determined within `hex_binning()`, if they are not provided. The results from various steps in the process are illustrated in Figure \@ref(fig:hex-bin). 

```{r, echo=TRUE, eval=FALSE}
hb_obj <- hex_binning(data = s_curve_noise_umap_scaled, x = "UMAP1", 
                      y = "UMAP2", num_bins_x = 8, 
                      num_bins_y = 14, x_start = NA, y_start = NA, 
                      buffer_x = NA, buffer_y = NA, hex_size = NA, 
                      col_start = "UMAP")

```

```{r hex-bin, out.width="100%", fig.cap = "Illustration of key steps of the hexagonal bining algorithm: (a) first, all hexagonal bin centroids are created; (b) second, full hexagon grid is created; and (c) finally, assign the data to hexagons. The centroids are used to generate hexagonal coordinates. Once the full hexagonal grid is generated, by computing the 2D euclidean distance from each NLDR points to hexagonal bin centroids, the nearest NLDR points are allocated to the nearest hexagoanl bin."}
...
The process of generating hexagonal bin centroids begins by defining the starting coordinates for the hexagons. With the number of bins computed along the x and y axes, along with the horizontal and vertical spacing, the centroids are computed by adding corresponding spacing to the hexagonal grid starting point along the axes.

The `gen_centroids()` function is used to compute all the centroids within the hexagonal grid after providing the number of bins along the x and y axes, hexagonal grid starting point, buffer size along the x and y axes, and hexagon size. 

```{r, echo=TRUE, eval=FALSE}
centroid_list <- gen_centroids(data = s_curve_noise_umap_scaled, x = "UMAP1", 
                               y = "UMAP2", num_bins_x = 8,
                               num_bins_y = 14, x_start = NA,
...
Once the centroids for the hexagonal grid are obtained, the next step is to compute the hexagonal coordinates. To obtain hexagonal coordinates by passing all the hexagonal centroids, `gen_hex_coord()` is used.

<!--write more on how grid coordinates are computed-->
...
Next, `compute_std_counts()` function is used to compute standardized number of points within each hexagon when provide the NLDR data along with their corresponding hexagonal IDs. 
...
To obtain hexagonal bin centroid coordinates, `extract_hexbin_centroids()`is used. The dataframes of all the centroids and the standardized counts for hexagons need to be provided. 
...
The function `extract_hexbin_mean()` is used to obtain mean value coordinates for each hexagon by passing the NLDR data along with their hexagonal IDs and standardized number of points within each hexagon. 
...
The `tri_bin_centroids()` is used to triangulate bin centroids/ bin means. Then, `gen_edges()` compute the edges to obtain the triangular mesh after providing the triangulated data.
...
The `avg_highd_data()` is used to generate $p-D$ model by passing the $p-D$ and binned NLDR data. The $p-D$ model for NLDR data is the averaged $p-D$ data within hexagons.
...
The `predict_emb()` function is used to predict 2D embedding for a new $p-D$ data point using the fitted model. This function is useful to predict 2D embedding irrespective of the NLDR technique.

In the prediction process, first, the nearest $p-D$ model point is identified for a given new $p-D$ point by computing $p-D$ Euclidean distance. Then, the corresponding $2-D$ bin centroid mapping for the identified $p-D$ model point is determined. Finally, the coordinates of the identified $2-D$ bin centroid is used as the predicted NLDR point for the new $p-D$ data point.
...
As a Goodness of fit statistics for the model, `gen_summary()` is used to compute residuals and MSE. These metrics are used to assess how well the fitted model will capture the underlying structure of the $p-D$ data. 
...
## Obtain UMAP
umap_pbmc <- umap(triangular_3d_data, n_neighbors = 35, n_components =  2,
...
```{r, echo=FALSE}
clusters_different_shapes_diff_num_points <- function(sample_size = 400, with_seed = NULL, cluster_size_vec = c(50, 50, 50, 50, 100, 100), num_gussian_clusters = 4, num_non_gaussian_clusters = 2, cluster_sd_gau = 0.05, cluster_sd_non_gau = 0.1, num_dims = 7, a = 2, b = 4) {
...
```{r}
clust_df <- clusters_different_shapes_diff_num_points(sample_size = 1500, with_seed = NULL, cluster_size_vec = c(250, 150, 150, 150, 350, 450), num_gussian_clusters = 4, num_non_gaussian_clusters = 2, cluster_sd_gau = 0.2, cluster_sd_non_gau = 0.3, num_dims = 5, a = 2, b = 4)
...
## Model with bin centroid
hex_size_vec <- seq(0.03, 2, by = 0.01)
...
mse_df_clust <- dplyr::bind_rows(not_dupli_df, duplicate_df) |>
  dplyr::mutate(type = "bin centroids")
```
...
mse_clust_plot <- ggplot(mse_df_clust, aes(x = num_bins,
  ...
  geom_point() +
  geom_line() +
   geom_vline(xintercept = 238, linetype="solid",
...
```{r, fig.cap="Parameter tuning plots, where the best choice of parameter at a large drop in MSE. Here, these are at *total number of bins* = 238."}
mse_clust_plot
```
...
## Decide by looking at MSE plot
num_bins_x_tsne_gau <- 14
num_bins_y_tsne_gau <- 27
hex_size_tsne_gau <- 0.05
x_start_tsne_gau <- -0.0481
y_start_tsne_gau <- 0.00833
## non-empty:80
hb_obj_tsne_gau <- hex_binning(data = tsne_gau_scaled, x = "tSNE1", 
                      y = "tSNE2", num_bins_x = num_bins_x_tsne_gau, 
                      num_bins_y = num_bins_y_tsne_gau, x_start = x_start_tsne_gau, y_start = y_start_tsne_gau,
...
model_object_tsne_gau <- fit_highd_model( training_data = clust_df, 
                                 ...
                                 x_start = x_start_tsne_gau, y_start = y_start_tsne_gau, 
                                 buffer_x = NA, buffer_y = NA, 
