Fit_PacMAP_data <- function(df_2_without_class, tem_dir){
  #write_csv(df_2_without_class, paste0(here::here(),"/df_2_without_class.csv"))
  write.csv(df_2_without_class, file.path(tem_dir, "df_2_without_class.csv"), row.names = FALSE,
            quote = TRUE)

  #file.path(temp_dir, "df_2_without_class.csv")
  #paste0(temp_dir, "df_2_without_class.csv")


  #file.copy(df_2_without_class, paste0(tem_dir, "\\", "df_2_without_class.csv"), recursive = TRUE)

  # write.csv(df_2_without_class, file.path(tempdir(), "df_2_without_class.csv"), row.names = FALSE,
  #           quote = TRUE)
  # drop_upload(file.path(tempdir(), "df_2_without_class.csv"), path = outputDir1)
}


### Plot tSNE in 2D

plot_PaCMAP_2D <- function(tSNE_df){
...
create_hex_bin <- function(tSNE_df, num_bins, shape){
  hb <- hexbin(tSNE_df$PaCMAP1, tSNE_df$PaCMAP2, num_bins, IDs = TRUE, shape = shape)
  return(hb)
}

### To Draw a hex bins with the points
draw_hex_bin <- function(tSNE_df, num_bins){
  hex_bin_plot <- ggplot(tSNE_df, aes(PaCMAP1, PaCMAP2)) +
