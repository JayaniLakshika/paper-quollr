### Fit PHATE without scaling and create the dataframe

Fit_PHATE <- function(df_2, knn){
  set.seed(200010151)
  tree_phate_fit <- phate(df_2, knn)
...
create_hex_bin <- function(PHATE_df, num_bins, shape){
  hb <- hexbin(PHATE_df$PHATE1, PHATE_df$PHATE2, num_bins, IDs = TRUE, shape = shape)
  return(hb)
}

### To Draw a hex bins with the points
draw_hex_bin <- function(PHATE_df, num_bins){
  hex_bin_plot <- ggplot(PHATE_df, aes(PHATE1, PHATE2)) +
...
create_hexbin_ID_for_all <- function(df_2, PHATE_df, num_bins, shape){
  ### Merge PHATE dataset in 2D with original dataset which contains 4D coordinates
  df_2 <- df_2 %>%
    mutate(ID=row_number())

  df_new <- df_2 %>% inner_join(PHATE_df, by = "ID")

  ### Fit hexbins and store hexbin IDs
  hb <- create_hex_bin(PHATE_df, num_bins, shape)
...
## To extract the dataframe with contain hexbin cetroids data and mean hexbin coordinate data(using hcell2xy())
#create_df_with_hex_bin_centroids_new <- function(df_b, PHATE_df){

### Fit hexbins and store hexbin IDs
#hb <- create_hex_bin(PHATE_df)
