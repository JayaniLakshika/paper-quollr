### Fit tSNE without scaling and create the dataframe

Fit_tSNE <- function(df_2, opt_perplexity, num_dim){
  set.seed(20001015)
  tSNE_fit <- df_2 %>%
    select(where(is.numeric)) %>%
    Rtsne(perplexity = opt_perplexity, pca = FALSE, pca_center = FALSE, normalize = FALSE, dims = num_dim)
...
find_avg_val <- function(tSNE_df){
  bw1 <- find_opt_bin_val(tSNE_df$tSNE1)
  bw2 <- find_opt_bin_val(tSNE_df$tSNE2)

  #tSNE1 <- data.frame(tSNE_val = tSNE_df$tSNE1)
  #tSNE2 <- data.frame(tSNE_val = tSNE_df$tSNE2)

  #tSNE1$type <- "tSNE1"
  #tSNE2$type <- "tSNE2"

  #cmb_tSNE <- rbind(tSNE1, tSNE2)

  #ggplot(cmb_tSNE, aes(tSNE_val, fill = type)) + geom_histogram(alpha = 0.2)
...
create_hex_bin <- function(tSNE_df, num_bins, shape){
  hb <- hexbin(tSNE_df$tSNE1, tSNE_df$tSNE2, num_bins, IDs = TRUE, shape = shape)
  return(hb)
}

### To Draw a hex bins with the points
draw_hex_bin <- function(tSNE_df, num_bins){
  hex_bin_plot <- ggplot(tSNE_df, aes(tSNE1, tSNE2)) +
...
plot_trimesh_in_ggplot <- function(tr1) { ## Parameter should be the tri.mesh object
  ...
  tri_mesh_plot <- ggplot(tr_df, aes(x = x, y = y)) +
    ...
    geom_point() +
    coord_equal() +
    ggtitle("Delaunay triangulation by tSNE") +
    theme(plot.title = element_text(hjust = 0.5, size = 18, face = "bold"))
...
plot_dist2 <- function(distance_df){
  ...
  dist_plotly <- ggplotly(dist_plot)%>%
    ...
    highlight(on = "plotly_selected", dynamic = TRUE, color = toRGB("red"), off = "plotly_doubleclick", persistent = FALSE) %>%
