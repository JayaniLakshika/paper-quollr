### Fit UMAP without scaling and create the dataframe

Fit_UMAP <- function(df_2, n_neighbors, n_components){
  set.seed(20001015)
  UMAP_fit <- df_2 %>%
...
find_avg_val <- function(UMAP_df){
  bw1 <- find_opt_bin_val(UMAP_df$UMAP1)
  bw2 <- find_opt_bin_val(UMAP_df$UMAP2)

  #UMAP1 <- data.frame(UMAP_val = UMAP_df$UMAP1)
  #UMAP2 <- data.frame(UMAP_val = UMAP_df$UMAP2)

  #UMAP1$type <- "UMAP1"
  #UMAP2$type <- "UMAP2"

  #cmb_UMAP <- rbind(UMAP1, UMAP2)

  #ggplot(cmb_UMAP, aes(UMAP_val, fill = type)) + geom_histogram(alpha = 0.2)

  #avg <- round((bw1 + bw2)/2,0)
  sum <- round(bw1 + bw2,0)
  return(sum)
}

## To create hexbin object
create_hex_bin <- function(UMAP_df, num_bins, shape){
  hb <- hexbin(UMAP_df$UMAP1, UMAP_df$UMAP2, num_bins, IDs = TRUE, shape = shape)
  return(hb)
}

### To Draw a hex bins with the points
draw_hex_bin <- function(UMAP_df, num_bins){
  hex_bin_plot <- ggplot(UMAP_df, aes(UMAP1, UMAP2)) +
...
create_hexbin_ID_for_all <- function(df_2, UMAP_df, num_bins, shape){
  ### Merge UMAP dataset in 2D with original dataset which contains 4D coordinates
  df_2 <- df_2 %>%
    mutate(ID=row_number())

  df_new <- df_2 #%>% inner_join(UMAP_df, by = "ID")

  ### Fit hexbins and store hexbin IDs
  hb <- create_hex_bin(UMAP_df, num_bins, shape)
  ...
  df <- df_new %>% select(starts_with("umap"), hb_id)
...
create_hexbin_avg_data <- function(df){
  ...
  ## Column names starts with x
  df_b <- df_b %>% select(starts_with("umap"), hb_id)
  return(df_b)
}
...
create_df_with_hex_bin_centroids <- function(df_b, hb){
  ...
  ### Fit hexbins and store hexbin IDs
  #hb <- create_hex_bin(UMAP_df, num_bins)
...
## To extract the dataframe with contain hexbin cetroids data and mean hexbin coordinate data(using hcell2xy())
#create_df_with_hex_bin_centroids_new <- function(df_b, UMAP_df){

### Fit hexbins and store hexbin IDs
#hb <- create_hex_bin(UMAP_df)
...
### Distance function
#calculate_distance <- function(UMAP_df, from_to_data_tri) { ## Parameter1: Dataframe which contain ID, x, and y values of the data points
...
#  result[i] <- sqrt(((UMAP_df[UMAP_df$ID  == from_to_data_tri$from[i],1] - UMAP_df[UMAP_df$ID == from_to_data_tri$to[i],1])^2) +
#                     ((UMAP_df[UMAP_df$ID == from_to_data_tri$from[i],2] - UMAP_df[UMAP_df$ID == from_to_data_tri$to[i],2])^2)) ## Calculate distance in 2D
...
get_langevitour_with_dist_criteria2 <- function(df, df_b, df_b_with_center_data, benchmark_value, distance_df){
...
get_langevitour2 <- function(df, df_b, df_predict, df_b_with_center_data){
  ...
  df_predict <- df_predict %>% mutate(type = "predict") ## prediction

  df_exe <- bind_rows(df_b, df, df_predict)
