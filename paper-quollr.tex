...
This paper presents the R package, \texttt{quollr}, which is useful for understanding how NLDR warps high-dimensional space and fits the data. Starting with an NLDR layout, our approach is to create a wireframe model representation, that can be lifted and displayed in the high-dimensional space (Figure \ref{fig:overview}).
...
\midrule
\textbackslash{}\$n, p, k\textbackslash{}\$ & number of observations, variables, embedding dimension, respectively\\
\$\textbackslash{}mathbfit\{X\}, \textbackslash{}mathbfit\{x\}\$ & \$p\$-dimensional data (population, sample)\\
\$\textbackslash{}mathbfit\{y\}\$ & \$k\$-dimensional layout\\
\$P\$ & orthonormal basis, generating a \$d\textbackslash{}text\{-\}dimensional\$ linear projection of \$p\$-dimensional data\\
\$T\$ & true  model\\
\addlinespace
\$g\$ & functional mapping from \textbackslash{}pD\{\} to \textbackslash{}kD\{\}, especially as prescribed by NLDR\\
\$\textbackslash{}mathbfit\{\textbackslash{}theta\}\$ & (Hyper-) parameters for NLDR method\\
\$r\$ & ranges of the embedding components\\
\$C\textasciicircum{}\{(j)\}\$ & \$j\$-dimensional bin centers\\
\$(b\_1, b\_2)\$ & number of bins in each direction\\
\addlinespace
\$(a\_1, a\_2)\$ & binwidths, distance between centroids in each direction\\
\$(s\_1, \textbackslash{} s\_2)\$ & starting coordinates of the hexagonal grid\\
