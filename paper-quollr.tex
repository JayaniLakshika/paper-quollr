...
We illustrate how to use \texttt{quollr} to assess whether this is a reasonable layout. Figure \ref{fig:limb-hbe} shows five alternative layouts, and the HBE plot summarizing the resulting model fits. Layout a is produce by UMAP (neighbors=5, minimum distance=0.1); layout c was produced by phate (knn=5); layout d was produced by trimap (number of inliers=12, outliers=4, random=3); layout e was produced by by pacmap (neighbors=10, init\_random\_MN-ratio=0.5, FP-ratio=2); layout f was produced by tSNE (perplexity=15).
