...
\begin{figure}[H]
\includegraphics[width=1\linewidth]{paper-quollr_files/figure-latex/unnamed-chunk-29-1} \caption{$2\text{-}D$ projections of the lifted high-dimensional wireframe model from the `Scurve` UMAP layout. Each panel (a1–a4) shows the model (blue) overlaid on `Scurve` data (black) in different projections. These views illustrate how the lifted wireframe model captures the structure of the `Scurve` data. The two twists visible in the UMAP layout can also be seen in the lifted model.}\label{fig:unnamed-chunk-29}
\end{figure}
...
\begin{figure}[H]
\includegraphics[width=0.5\linewidth]{figures/model_proj1_detourr} \includegraphics[width=0.5\linewidth]{figures/model_proj2_detourr} \caption{Screenshots of the lifted high-dimensional wireframe model from the `Scurve` UMAP layout using `detourr`.}\label{fig:unnamed-chunk-30}
\end{figure}
