...
For each centroid, six vertices are computed and assigned a polygon ID (\texttt{hex\_poly\_id}) corresponding to the centroid's \texttt{h}. These points are aggregated into a tibble that includes the polygon ID (\texttt{hex\_poly\_id}) and the respective x (\texttt{x}) and y (\texttt{y}) coordinates for all hexagon corners.
