...
The following demonstration of the package's functionality assumes \texttt{quollr} has been loaded. To begin the workflow, user needs two inputs: the high-dimensional dataset and the corresponding NLDR data. The high-dimensional data must contain a unique \texttt{ID} column, with data columns prefixed by the letter \texttt{"x"} (e.g., \texttt{x1}, \texttt{x2}, etc.). The NLDR dataset should include embedding coordinates labeled as \texttt{emb1} and \texttt{emb2}, ensuring one-to-one correspondence with the high-dimensional data through the shared \texttt{ID}.
...
For this example, we use the UMAP layout, which is used \texttt{n\_neighbors\ =} \(46\) and \texttt{min\_dist\ =} \(0.9\) as hyper-parameters.
...
The output is a tibble containing a unique hexagon ID (\texttt{h}) and centroid coordinates (\texttt{c\_x}, \texttt{c\_y}), which together define the layout of the complete hexagonal grid over the \(2\text{-}D\) space.
