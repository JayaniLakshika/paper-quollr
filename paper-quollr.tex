...
The \texttt{scurve} is a \(7\text{-}D\) simulated dataset. It is constructed by simulating \(1000\) observations.

\texttt{scurve\_umap} is the UMAP $2\text{-}D$ embedding for \texttt{scurve} data with \texttt{n\_neighbors}\(=46\) and \texttt{min\_dist}\(=0.9\). Each data set contains a unique ID column that maps the \(7\text{-}D\) point in \texttt{scurve} to the corresponding $2\text{-}D$ \texttt{scurve\_umap}.
