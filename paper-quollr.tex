...
The remaining variables \(X_4\), \(X_5\), \(X_6\), \(X_7\) are all uniform error, with small variance. \texttt{scurve\_umap} is the UMAP \(2\text{-}D\) embedding for \texttt{scurve} data with \texttt{n\_neighbors}\(=46\) and \texttt{min\_dist}\(=0.9\). Each data set contains a unique ID column that maps the \(7\text{-}D\) point in \texttt{scurve} to the corresponding \(2\text{-}D\) \texttt{scurve\_umap}.
...
The algorithm starts by scaling the NLDR data to to the range \([0, 1] \times [0, x_2]\), where \(x_2\) is the maximum value of the second embedding, using the \texttt{gen\_scaled\_data()} function. The output includes the scaled NLDR data (\texttt{scaled\_nldr}) along with the original limits of the embeddings (\texttt{lim1}, \texttt{lim2}).
...
The configurations of a hexagonal grid are determined by the number of bins and the bin width in each direction. The function \texttt{calc\_bins\_y()} is used for this purpose. This function accepts (1) an object containing three elements: the first is the scaled NLDR data with two columns, and the second and third are the limits of the original NLDR data; (2) the number of bins along the x-axis (\texttt{b1}), and (3) the buffer amount as a proportion (\texttt{q}).
