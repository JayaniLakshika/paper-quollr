...
\begin{figure}[H]
\includegraphics[width=1\linewidth]{figures/quollr_workflow} \caption{Workflow for }\label{fig:workflow}
\end{figure}
...
\item
  Construct $2\text{-}D$ model --- \texttt{hex\_binning()}, \texttt{merge\_hexbin\_centroids()}, \texttt{tri\_bin\_centroids()}, \texttt{gen\_edges()}, \texttt{update\_trimesh\_index()}
\item
  Lift the model into $p\text{-}D${} --- the \texttt{avg\_highd\_data()}
\item
  Prediction - \texttt{predict\_emb()}
\item
  Error computation --- \texttt{glance()}
\item
  Visualization --- \texttt{geom\_hexgrid()}, \texttt{geom\_trimesh()}, \texttt{show\_langevitour()}
\item
  Interactivity - \texttt{show\_link\_plots()}, and \texttt{show\_error\_link\_plots()}
\end{enumerate}
