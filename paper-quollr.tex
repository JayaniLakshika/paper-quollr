...
\subsubsection{\texorpdfstring{\(2\text{-}D\) projection}{2\textbackslash text\{-\}D projection}}\label{text-d-projection}
...
The function \texttt{show\_link\_plots()} generates two side-by-side, interactively linked plots; a 2D scatterplot of the NLDR representation, and a dynamic projection tour in the original high-dimensional space (using the \texttt{langevitour} package), displaying both the data and the model.
...
\section{Find the most appropriate fit}\label{find-the-most-appropriate-fit}

\includegraphics[width=1\linewidth]{paper-quollr_files/figure-latex/unnamed-chunk-33-1}
